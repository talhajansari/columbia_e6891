...
for i=1:N,
    ...
    refresh_mfcc_calls = false;
    if exist(strcat(dir_mfcc_calls, info{i,2},'.mat'), 'file')==0 
        refresh_mfcc_calls = true;
    else 
        mfcc_c = load(strcat(dir_mfcc_calls, info{i,2},'.mat'));
        mfcc_c = mfcc_c.X;
        if isempty(mfcc_c{1})
            refresh_mfcc_calls = true;
            disp(strcat('- E: CallMFCC:', info{i,2}));
        end
    end
    for j=1:N_calls % For each call
        %disp(strcat('- - - In Call:',int2str(j),'/',int2str(N_calls)));
        
        if refresh_mfcc_calls==true
        ...
        if exist(strcat(dir_pulses_calls, info{i,2},'_',int2str(j),'.mat'), 'file')==0   
            ...
            pulses = pulses.Y;
            if isempty(pulses)
                disp(strcat('E: Pulse:',info{i,2},'_',num2str(j)));
                pulses = seg2pulse(calls{j}, fs, 1000, dir_pulses_calls, false, 'segment');
                Y=pulses;
                save(strcat(dir_pulses_calls, info{i,2},'_',int2str(j),'.mat'), 'Y');
            end
