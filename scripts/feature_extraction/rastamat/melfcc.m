function [aspectrum,pspectrum,logE, spec] = melfcc(samples, sr, varargin)
...
[pspectrum,logE, spec] = powspec(samples, sr, wintime, hoptime, dither);

I2 = medianClipping(pspectrum, 3, 3, 1);

aspectrum = audspec(I2, sr, nbands, fbtype, minfreq, maxfreq, sumpower, bwidth);
mx = max(aspectrum(:))/1e5;

aspectrum = medianClipping(aspectrum, 3, 3, 1);
min(aspectrum(:));
if (usecmp)
