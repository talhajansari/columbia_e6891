...
if (length(nfft)==1) || use_chirp
   ...
   for i = 1:20
       try
           idx = rowindex64(:,ones(1,ncol))+colindex64(ones(nwind,1),:)-1;
            y(:) = x(idx);
       catch err
           disp('index error again')
           if i < 20
               %imagesc(idx);
               sort(idx);
...
%                    rowindex0 = rowindex;
%                    colindex0 = colindex;
%                elseif strcmp(err.message, 'Subscript indices must either be real positive integers or logicals.')
%                    rowindex0 = rowindex64;
%                    colindex0 = colindex64;
%                end
               continue
           else
               rethrow(err)
